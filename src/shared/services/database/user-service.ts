...
class UserService {
  public async addUserToDB(data: IUserDoc): Promise<void> {
    await UserModel.create(data);
  }

  public async getUserByAuthId(authId: string): Promise<IUserDoc> {
    const users: IUserDoc[] = await UserModel.aggregate([
  ...
  public async getUserById(userId: string): Promise<IUserDoc> {
    // using the aggregate method is more efficient and gives
    // more options to format the result
    // $unwind will convert the returned List $lookup as an object
    const users: IUserDoc[] = await UserModel.aggregate([
  ...
  private aggregateProject() {
    // any property you want returned, just set it to 1
    return {
      _id: 1,
      username: '$authId.username',
      uId: '$authId.uId',
